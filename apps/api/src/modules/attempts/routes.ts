...
export const attemptRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch<{ Params: { id: string }; Body: Record<string, unknown> }>("/attempts/:id/answers", async (request, reply) => {
    const auth = await requireUser(request, reply);
    if (!auth) return;
    ...
    const normalized = incoming.map((raw) => {
      ...
      const examQuestionId = typeof answer.examQuestionId === "string" ? answer.examQuestionId : "";
      const selectedChoice = answer.selectedChoice == null ? null : String(answer.selectedChoice).toUpperCase();
      ...
      return { examQuestionId, selectedChoice, flagged, timeSpentSeconds, clientRevision };
    });
    ...
    const existing = await db.select({
      ...
      .where(and(
        ...
        inArray(schema.attemptAnswers.examQuestionId, questionIds)
      ));

    const existingByQuestion = new Map(existing.map((answer) => [answer.examQuestionId, answer]));

    for (const answer of normalized) {
      const current = existingByQuestion.get(answer.examQuestionId);
      if (current && current.clientRevision > answer.clientRevision) continue;
      ...
      if (current) {
        await db.update(schema.attemptAnswers).set({
          ...
          savedAt: new Date()
        }).where(eq(schema.attemptAnswers.id, current.id));
      } else {
